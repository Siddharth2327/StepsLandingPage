...
// Pre-computed stars — static data, never recreated
...
// Seeded RNG — pure function, no side effects
function rng(seed) {
...
// ─── Static scene data (outside component — never recreated) ──────────────────
const SETBACKS = [
...
// CSS injected once at module level — never causes style recalc on re-render
const GLOBAL_CSS = `
...
.cta-p{display:inline-flex;align-items:center;gap:9px;padding:13px 26px;border-radius:12px;border:none;cursor:pointer;
  ...
  transition:transform .3s,filter .3s;color:#0A1228;}
...
.cta-g{display:inline-flex;align-items:center;gap:9px;padding:13px 26px;border-radius:12px;cursor:pointer;
  ...
  transition:background .3s,transform .3s;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.22);
...
/* CSS-driven star twinkle — zero JS per frame */
@keyframes twinkle{0%,100%{opacity:0.35}50%{opacity:0.95}}
.star{animation:twinkle var(--tw) ease-in-out infinite;animation-delay:var(--delay);}
/* CSS-driven aviation light blink */
@keyframes avblink{0%,100%{opacity:1}45%,55%{opacity:0.08}}
.av-light{animation:avblink 2s ease-in-out infinite;}
.av-light-b{animation:avblink 2.3s ease-in-out infinite 0.9s;}
`;

// Inject global CSS exactly once
...
// ─── STARS — pure CSS animation, zero Framer Motion instances ────────────────
const StarsLayer = memo(function StarsLayer({ opacity }) {
  return (
      ...
      <svg width="100%" height="58%" viewBox="0 0 1440 420" preserveAspectRatio="xMidYMid meet">
        {STARS.map((s, i) => (
          <circle
            ...
            cx={`${s.x}%`} cy={`${s.y}%`} r={s.r}
            fill="white"
            style={{ "--tw": `${s.tw}s`, "--delay": `${s.delay}s` }}
          />
        ))}
      </svg>
    </div>
...
// ─── MOON ─────────────────────────────────────────────────────────────────────
const Moon = memo(function Moon({ opacity }) {
  return (
...
// ─── SUN ──────────────────────────────────────────────────────────────────────
const Sun = memo(function Sun({ tod }) {
  if (tod.sunSize <= 0) return null;
  const sz = tod.sunSize;
  return (
    <div className="absolute pointer-events-none"
      style={{
        left: `${tod.sunX}%`, top: `${tod.sunY}%`,
      ...
      {tod.id === "day" && (
        <div style={{
          position: "absolute", inset: -sz * 0.8, borderRadius: "50%",
...
// ─── CLOUDS ───────────────────────────────────────────────────────────────────
const CloudLayer = memo(function CloudLayer({ tod, scrollX }) {
  ...
  return (
    <motion.div className="absolute inset-0 pointer-events-none overflow-hidden"
      style={{ zIndex: 10, x: scrollX, willChange: "transform" }}>
      {CLOUDS.map((c, i) => (
        <motion.div key={i} className="absolute"
          style={{
            left: `${c.x}%`, top: `${c.y}%`, width: `${32 * c.s}%`,
...
// ─── KL SKYLINE ───────────────────────────────────────────────────────────────
// memo: only re-renders when tod reference changes (once per scene transition)
const KLSkylineLayer = memo(function KLSkylineLayer({ tod }) {
  ...
  // Window grids are memoized per tod.id — only recomputed on scene change
  const winData = useMemo(() => {
    function buildWins(bx, by, bw, bh, seed) {
      const r = rng(seed);
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          wins.push({
            ...
            y: by + row * wy + wy * 0.2,
            w: wx * 0.55, h: wy * 0.48,
            lit: r() < (tod.windowLit + (isNight ? 0.48 : 0)),
          });
        }
      }
      return wins;
    ...
    return {
      petroL: buildWins(-26, 70, 52, 263, 71),
      petroR: buildWins(765, 70, 52, 263, 83),
      klt:    buildWins(497, 175, 20, 185, 91),
      ex6:    buildWins(879, 67, 56, 296, 106),
      tel:    buildWins(379, 109, 48, 250, 55),
      max:    buildWins(293, 149, 42, 214, 44),
      mt13:   buildWins(199, 179, 40, 184, 13),
      mt27:   buildWins(249, 205, 36, 158, 27),
      mt38:   buildWins(1017,172, 46, 191, 38),
      mt49:   buildWins(1073,195, 40, 168, 49),
      mt61:   buildWins(1129,157, 52, 206, 61),
      mt74:   buildWins(945, 129, 54, 234, 74),
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tod.id]);

  function WinsFromData({ data }) {
    return data.map((w, i) => (
      <rect key={i} x={w.x} y={w.y} width={w.w} height={w.h} rx={0.4}
        fill={w.lit ? winLit : winDark} opacity={w.lit ? 1 : 0.45}/>
    ));
  }
  ...
  function PetronasTowers() {
    ...
    function Tower({ cx, side }) {
      ...
      const x = cx - 28;
      const wins = side === "L" ? winData.petroL : winData.petroR;
      const winsOffset = side === "L" ? 0 : (cx - 28 - (-26));
      ...
      return (
        <g>
          ...
          {isNight && (
            <ellipse cx={cx} cy={358} rx={70} ry={12}
              fill="rgba(80,130,255,0.08)" opacity="0.6"/>
          )}
          ...
          {/* Pre-computed window rects */}
          {wins.map((w, i) => (
            <rect key={i}
              x={side === "L" ? w.x : w.x + winsOffset}
              y={w.y}
              width={w.w} height={w.h} rx={0.4}
              fill={w.lit ? winLit : winDark} opacity={w.lit ? 1 : 0.45}/>
          ))}
          ...
          {/* CSS-animated aviation light — no Framer Motion */}
          <circle cx={cx} cy={2} r={2.8} fill="rgba(255,48,48,0.96)"
            className={side === "L" ? "av-light" : "av-light"}
            style={{ animationDelay: side === "L" ? "0s" : "0.55s" }}/>
          {isNight && (
            <ellipse cx={cx} cy={2} rx={9} ry={9} fill="rgba(255,48,48,0.18)" opacity="0.8"/>
          )}
    ...
    return (
      <g>
        ...
        {isNight && (
          <rect x={bx1} y={210} width={bx2 - bx1} height={14}
            fill="rgba(100,150,255,0.07)" rx={3} opacity="0.8"/>
        )}
      </g>
    );
  }
    ...
    return (
      <g>
        ...
        {Array.from({ length: 8 }, (_, i) => {
          ...
          return (
            <rect key={i} x={wx - 4} y={wy - 4} width={8} height={7} rx={1}
              fill={isNight ? "rgba(255,215,145,0.85)" : (amb > 0.7 ? glassTint : "rgba(180,220,255,0.5)")}/>
          );
        })}
        {isNight && <ellipse cx={tx} cy={168} rx={40} ry={34} fill="rgba(100,155,255,0.08)" opacity="0.9"/>}
        ...
        <circle cx={tx} cy={4} r={2.5} fill="rgba(255,50,50,0.96)" className="av-light-b"/>
        {isNight && <ellipse cx={tx} cy={4} rx={8} ry={8} fill="rgba(255,50,50,0.16)" opacity="0.9"/>}
        {isNight && <ellipse cx={tx} cy={340} rx={50} ry={10} fill="rgba(80,130,255,0.06)" opacity="0.8"/>}
      </g>
    );
  }
  ...
  function Exchange106() {
    ...
    return (
      <g>
        ...
        <circle cx={x + w / 2} cy={365 - h - 18} r={2.5} fill="rgba(255,48,48,0.95)"
          className="av-light" style={{ animationDelay: "0.4s" }}/>
        {isNight && <ellipse cx={x + w / 2} cy={365 - h / 2} rx={50} ry={h * 0.45} fill="rgba(80,125,255,0.05)" opacity="0.9"/>}
      </g>
    );
  }
  ...
  function MenaraTelekom() {
    ...
    return (
      <g>
        ...
        <circle cx={x + w / 2} cy={365 - h - 12} r={2.3} fill="rgba(255,48,48,0.95)"
          className="av-light" style={{ animationDelay: "1.5s" }}/>
        {isNight && <ellipse cx={x + w / 2} cy={365 - h / 2} rx={42} ry={h * 0.42} fill="rgba(80,125,255,0.05)" opacity="0.9"/>}
      </g>
    );
  }
  ...
  function MenaraMaxis() {
    ...
    return (
      <g>
        ...
        <circle cx={x + w / 2} cy={365 - h - 18} r={2.2} fill="rgba(255,48,48,0.94)"
          className="av-light" style={{ animationDelay: "0.7s" }}/>
        {isNight && <ellipse cx={x + w / 2} cy={365 - h / 2} rx={38} ry={h * 0.42} fill="rgba(80,125,255,0.05)" opacity="0.9"/>}
      </g>
    );
  }

  function ModernTower({ x, h, w, seed, flip = false, avDelay = 0, winsKey }) {
    const gid = `mt${x}`, rfid = `mtr${x}`;
    const wins = winData[winsKey] || [];
    return (
      <g>
        <defs>
          <linearGradient id={gid} x1="0" y1="0" x2="1" y2="0">
        ...
        {[h, h * 0.75, h * 0.48, h * 0.22].map((sh, i) =>
          i > 0 ? (
            <rect key={i} x={x + w * i * 0.06} y={365 - sh} width={w * (1 - i * 0.12)} height={5}
              fill={amb > 0.4 ? "#182840" : "#070F1E"} opacity="0.85" rx={0.5}/>
          ) : null
        )}
        {wins.map((win, i) => (
          <rect key={i} x={win.x} y={win.y} width={win.w} height={win.h} rx={0.4}
            fill={win.lit ? winLit : winDark} opacity={win.lit ? 1 : 0.45}/>
        ))}
        ...
        <circle cx={x + w / 2} cy={365 - h - 20} r={2.2} fill="rgba(255,48,48,0.94)"
          className="av-light" style={{ animationDelay: `${avDelay}s` }}/>
        {isNight && <ellipse cx={x + w / 2} cy={365 - h / 2} rx={w * 0.8} ry={h * 0.42} fill="rgba(80,125,255,0.05)" opacity="0.9"/>}
      </g>
    );
  }
  ...
  function MallPodium() {
    return (
      <g>
        ...
        <rect x={577} y={308} width={282} height={2}
          fill={amb > 0.4 ? "rgba(100,160,230,0.2)" : "rgba(80,130,220,0.15)"}/>
      </g>
    );
  }

  // Pre-compute background building window data
  const bgWinData = useMemo(() => BG_BUILDINGS.map((b, i) => {
    const br = rng(i * 7 + 3);
    ...
    for (let row = 0; row < wr; row++) {
      for (let col = 0; col < wc; col++) {
        wins.push({
          ...
          w: (b.w / wc) * 0.55,
          h: (b.h / wr) * 0.5,
          lit: br() < (tod.windowLit + (isNight ? 0.42 : 0)),
        });
      }
    }
    return wins;
  ...
  const midWinData = useMemo(() => MID_BUILDINGS.map(b => {
    const mr = rng(b.s);
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        wins.push({
          x: b.x + 2 + col * 10,
          y: b.y + 3 + row * 13,
          lit: mr() < (tod.windowLit + (isNight ? 0.5 : 0)),
        });
      }
    }
    return wins;
    ...
    <svg viewBox="0 0 1440 380" preserveAspectRatio="xMidYMax meet"
      ...
      {BG_BUILDINGS.map((b, i) => (
        <g key={`bg${i}`} opacity={0.7 + amb * 0.15}>
          ...
          {bgWinData[i].map((w, j) => (
            <rect key={j} x={w.x} y={w.y} width={w.w} height={w.h} rx={0.4}
              fill={w.lit ? winLit : winDark} opacity={w.lit ? 0.82 : 0.38}/>
          ))}
        </g>
      ))}
      ...
      {MID_BUILDINGS.map((b, i) => (
        <g key={`mid${i}`}>
          ...
          {midWinData[i].map((w, j) => (
            <rect key={j} x={w.x} y={w.y} width={7} height={7} rx={0.4}
              fill={w.lit ? winLit : winDark} opacity={w.lit ? 1 : 0.38}/>
          ))}
        </g>
      ))}

      <ModernTower x={197} h={188} w={44} seed={13} avDelay={1.2} winsKey="mt13"/>
      <ModernTower x={247} h={162} w={40} seed={27} flip avDelay={0.6} winsKey="mt27"/>
      <ModernTower x={1015} h={195} w={50} seed={38} avDelay={1.6} winsKey="mt38"/>
      <ModernTower x={1071} h={172} w={44} seed={49} flip avDelay={0.3} winsKey="mt49"/>
      <ModernTower x={1127} h={210} w={56} seed={61} avDelay={2.1} winsKey="mt61"/>

      <MenaraMaxis/>
      <MenaraTelekom/>
      <KLTower/>
      <Exchange106/>
      <ModernTower x={943} h={238} w={58} seed={74} avDelay={0.9} winsKey="mt74"/>
      <MallPodium/>
      <PetronasTowers/>
      ...
      {isNight && (
        <>
          <ellipse cx={647} cy={352} rx={160} ry={18} fill="rgba(80,130,255,0.07)" opacity="0.9"/>
          <ellipse cx={793} cy={352} rx={160} ry={18} fill="rgba(80,130,255,0.07)" opacity="0.9"/>
          <ellipse cx={507} cy={350} rx={80}  ry={12} fill="rgba(80,130,255,0.06)" opacity="0.9"/>
          <ellipse cx={907} cy={352} rx={120} ry={15} fill="rgba(80,130,255,0.06)" opacity="0.9"/>
        </>
      )}
    </svg>
  );
...
export default function Home() {
  ...
  // Use refs for RAF loop internals to avoid stale closures
  ...
  // RAF-based timer — pauses when tab is hidden, no drift
  useEffect(() => {
    ...
    function tick(now) {
      ...
      if (elapsed >= total - FADE_MS && !fadingRef.current) {
      ...
      if (elapsed >= total) {
        ...
        setFading(false);
        setPrevIndex(null);
        return; // new effect will start fresh
      }
  ...
  // Memoized sky gradient — only recomputed on scene change
  const skyGrad = useMemo(() => {
    const stops = tod.sky.map((c, i) => `${c} ${tod.skyStops[i]}`);
    return `linear-gradient(180deg,${stops.join(",")})`;
  }, [tod.id]); // eslint-disable-line

  const prevSkyGrad = useMemo(() => {
    if (!prevTod) return null;
    const stops = prevTod.sky.map((c, i) => `${c} ${prevTod.skyStops[i]}`);
    return `linear-gradient(180deg,${stops.join(",")})`;
  }, [prevTod?.id]); // eslint-disable-line
  ...
  const handleTodClick = useCallback((i) => {
    setTodIndex(i);
    setProgress(0);
    setFading(false);
    setPrevIndex(null);
  ...
  const ctaBg = useMemo(() => `linear-gradient(135deg,${
  ...
  return (
    <section ref={containerRef} className="kl2 relative w-full overflow-hidden"
      ...
      <motion.div className="absolute inset-0"
        style={{ background: skyGrad, y: bgY, zIndex: 1, transition: "background 3.5s ease", willChange: "transform" }}/>

      {/* CROSSFADE OVERLAY */}
      {prevTod && fading && prevSkyGrad && (
        <motion.div className="absolute inset-0 pointer-events-none"
          style={{ background: prevSkyGrad, y: bgY, zIndex: 2, willChange: "transform" }}
          initial={{ opacity: 1 }}
          animate={{ opacity: 0 }}
      ...
      {/* SKYLINE */}
      <motion.div className="absolute inset-0 pointer-events-none"
        style={{ zIndex: 22, y: buildingsY, willChange: "transform" }}
      ...
      <motion.div className="absolute inset-0 flex items-end justify-center pointer-events-none"
        style={{ zIndex: 18, y: textY, willChange: "transform" }}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      <motion.div className="absolute inset-0 flex items-center justify-center px-6"
        style={{ zIndex: 40, y: contentY, willChange: "transform" }}>
        <div style={{ textAlign: "center", maxWidth: 760 }}>
          ...
          <motion.div
            ...
            Welcome to&nbsp;
            <span style={{
              fontWeight: 700, color: gold, letterSpacing: ".22em",
          ...
          <motion.div
            ...
            <button className="cta-p" style={{ background: ctaBg }} onClick={() => scrollTo("services")}>
              Explore Services
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 26 }}>
          {[["20+", "Years"], ["500+", "Clients"], ["15+", "Countries"]].map(([num, lbl], i) => (
            <div key={lbl} style={{ display: "contents" }}>
              {i > 0 && <div className="divdr"/>}
              <div className="stat-i">
                <span style={{
                  fontFamily: "'Cormorant Garamond',serif", fontSize: 26,
                  fontWeight: 700, color: gold, lineHeight: 1, transition: "color 2.5s ease",
                }}>{num}</span>
                <span style={{
                  fontFamily: "'Jost',sans-serif", fontSize: 10.5, color: subColor,
                  letterSpacing: ".2em", textTransform: "uppercase", marginTop: 4,
                }}>{lbl}</span>
              </div>
            </div>
          ))}
      ...
      {/* SCROLL CUE */}
      <motion.div
        className="absolute bottom-6 left-1/2 flex flex-col items-center gap-1.5 cursor-pointer"
